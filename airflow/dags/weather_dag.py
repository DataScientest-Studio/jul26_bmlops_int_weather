...
with DAG(
    ...
    ingestion = DockerOperator(
        ...
        mounts=[
            Mount(source=path + "/data", target="/app/data", type="bind")
            ],
    ...
    training = DockerOperator(
        ...
        environment = {"SUPABASE_URL": os.environ.get("SUPABASE_URL"), 
    ...
    evaluation = DockerOperator(
        ...
        network_mode = "jul26_bmlops_int_weather_weather_network",
        auto_remove = True,
        command = "python3 -m weather_mlops.models.evaluation --x-data data/processed/X_validation.csv --y-data data/processed/y_validation.csv --metrics-output reports/metrics/validation.json --split-name validation",
        mount_tmp_dir=False
        )
